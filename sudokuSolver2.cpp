...
int main(int argc, char* argv[]) {	
	...
	while (countUnsolved > 0) {
		...
		//Solver for cells where there is only one possible value
		for (int row = 1; row <= 9; row++) {
			for (int col = 1; col <= 9; col++) {
		...
		//Solver for cells where number can not go anywhere else
		for (int guess = 1; guess <= 9; guess++) {
			//Check all group cells
			for (int o_row = 1; o_row <= 7; o_row += 3) {
				for (int o_col = 1; o_col <= 7; o_col += 3) {
					countPossible[guess] = 0;
					//Check within each group cell
					for (int off_row = 0; off_row <= 2; off_row++) {
						for (int off_col = 0; off_col <= 2; off_col++) {
							if (gameBoard[o_row + off_row][o_col + off_col].getValue() == guess) {
								countPossible[guess] = 9;
							}
							if (gameBoard[o_row + off_row][o_col + off_col].hasNote(guess)) {
								countPossible[guess]++;
								tempRow = o_row + off_row;
								tempCol = o_col + off_col;
							}
						}
					}
					//If only one possibility for the number within the group cell, place it
					if (countPossible[guess] == 1) {
						gameBoard[tempRow][tempCol].setValue(guess);
						//Let related cells know that the value is no longer possible
						updateNotes(gameBoard, tempRow, tempCol, gameBoard[tempRow][tempCol].getValue());
						//Decrement number of unsolved cells
						countUnsolved--;
						//Update readout
						std::cout << ".";
			...
			for (int row = 1; row <= 9; row++) {
				countPossible[guess] = 0;
				for (int col = 1; col <= 9; col++) {
					if (gameBoard[row][col].getValue() == guess) {
						countPossible[guess] = 9;
					}
					if (gameBoard[row][col].hasNote(guess)) {
						countPossible[guess]++;
						tempRow = row;
						tempCol = col;
					}
				}
				//If only one possibility for the number within the row, place it
				if (countPossible[guess] == 1) {
					gameBoard[tempRow][tempCol].setValue(guess);
					//Let related cells know that the value is no longer possible
					updateNotes(gameBoard, tempRow, tempCol, gameBoard[tempRow][tempCol].getValue());
			...
			for (int col = 1; col <= 9; col++) {
				...
				//If only one possibility for the number within the column, place it
				if (countPossible[guess] == 1) {
					gameBoard[tempRow][tempCol].setValue(guess);
					//Let related cells know that the value is no longer possible
					updateNotes(gameBoard, tempRow, tempCol, gameBoard[tempRow][tempCol].getValue());
